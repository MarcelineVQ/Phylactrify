...
local function reequip()
	...
	if b and s then
		...
		if RegearSettings.announce then
			amprint("Could not find " .. RegearSettings.item_link)
		end
	end
...
local function OnEvent()
	if RegearSettings.do_toggle
		...
		-- delay 500ms to avoid race condition looking for item in bags
		Regear.bag_update_delay = GetTime() + 0.5
		Regear:SetScript("OnUpdate", function()
			if Regear.bag_update_delay < GetTime() then
				-- delay is over, unregister event and reequip
				Regear:SetScript("OnUpdate", nil)
				reequip()
			end
		end)
...
local function Init()
	if event == "ADDON_LOADED" and arg1 == "Regear" then
		...
		then
			RegearSettings = defaults -- initialize default settings
		else
			-- or check that we only have the current settings format
			local s = {}
